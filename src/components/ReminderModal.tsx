      ...
      <Card className="w-full max-w-md shadow-2xl rounded-3xl overflow-hidden animate-in zoom-in-95 duration-200 border-0">
        <CardHeader className="bg-gradient-to-br from-sky-500 to-indigo-600 p-6">
          ...
          <CardDescription className="text-sky-100 mt-1">
            Relembre o pessoal sobre {category === 'loan' ? 'esta parcela do empréstimo' : 'esta contribuição da meta'}. Escolha o destinatário:
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 bg-white space-y-3">
          ...
          >
            ...
            <div className="text-left font-normal flex-1">
              <span className="block font-bold text-slate-900 group-hover:text-emerald-800">Enviar para {nameP1}</span>
              <span className="text-xs text-slate-500">{phoneP1 ? `WhatsApp: ${phoneP1}` : 'Nenhum WhatsApp cadastrado. Abrirá lista de contatos.'}</span>
            </div>
          </Button>
          ...
          {goalType === 'shared' && (
            ...
            >
              ...
              <div className="text-left font-normal flex-1">
                <span className="block font-bold text-slate-900 group-hover:text-emerald-800">Enviar para {nameP2}</span>
                <span className="text-xs text-slate-500">{phoneP2 ? `WhatsApp: ${phoneP2}` : 'Nenhum WhatsApp cadastrado. Abrirá lista de contatos.'}</span>
              </div>
            </Button>
          )}
          ...
          >
            ...
            <div className="text-left font-normal flex-1">
              <span className="block font-bold text-slate-900">Copiar Texto do Lembrete</span>
              <span className="text-xs text-slate-500">Copia o resumo para colar em outro app</span>
            </div>
          </Button>
        </CardContent>
        <CardFooter className="flex justify-center border-t border-slate-100 bg-slate-50 p-4">
          <Button variant="ghost" className="w-full rounded-2xl h-12 text-slate-500 hover:bg-slate-200/50" onClick={onClose}>
            Cancelar
          </Button>
        </CardFooter>
      </Card>
