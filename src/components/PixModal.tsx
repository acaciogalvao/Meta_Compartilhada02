      ...
      <div className="glass-card w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl animate-in slide-in-from-bottom-full sm:zoom-in duration-300 flex flex-col max-h-[90vh] pb-6 border border-white/10">
        <div className="p-6 pb-2">
          <div className="flex justify-between items-center mb-1">
            ...
            <button onClick={() => { setShowPixModal(false); setPixCode(""); setPixAmount(""); }} className="text-slate-400 hover:text-white transition-colors bg-white/5 rounded-full p-1">
              <X className="w-6 h-6" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 border-t border-white/10 pt-6">
          ...
          {paymentSuccess ? (
            <div className="flex flex-col items-center justify-center py-8 text-emerald-400 space-y-4">
              ...
              <p className="text-xl font-bold shadow-emerald-400 text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-sky-400">Pagamento Confirmado!</p>
              <p className="text-sm text-slate-400 text-center">O valor já foi adicionado à sua meta.</p>
              <Button 
                variant="outline" 
            ...
            <div className="flex flex-col items-center mt-2">
              <p className="text-sm text-slate-400 mb-6">Escaneie para pagar para <span className="font-semibold text-white">{payerName}</span></p>
              ...
              <div className="bg-white p-2 rounded-2xl shadow-[0_0_20px_rgba(255,255,255,0.1)] border border-white/20 mb-6 inline-block relative">
                <img src={`data:image/png;base64,${qrCodeBase64}`} alt="QR Code Pix" className="w-48 h-48 rounded-xl" />
              </div>
              ...
              <div className="w-full space-y-3">
                <Button 
                  ...
                  onClick={copyPixCode}
                >
                  {copied ? <CheckCircle2 className="w-5 h-5 mr-2 text-emerald-400" /> : <Copy className="w-5 h-5 mr-2" />}
